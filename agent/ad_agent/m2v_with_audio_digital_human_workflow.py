...
from agent.third_part.tiktok import create_tt_digital_human_list
import shutil
...
async def generate_video(state: GenerateVideoState, config):
    ...
    async def process_video_fragment(video_fragment: VideoFragment):
        ...
        with open(state.get_real_url(video_url_v1), "wb") as f:
            f.write(video_data)
        video_fragment.video_url_v1 = video_url_v1
        # 获取视频时长
        video_duration = get_video_duration(state.get_real_url(video_url_v1))
...
async def generate_audio_text(state: GenerateVideoState, config):
    ...
    if state.is_audio_too_long:
        ...
        # 6-9  8s-> 12  - 18
        word_max_count = int((state.video_fragment_duration-2)*3)
    else:
...
async def generate_digital_human_video_and_audio(state: GenerateVideoState, config):
    ...
    video_scripts: list[str] = [
        video_fragment.video_script for video_fragment in state.video_fragments]
    video_urls = await create_tt_digital_human_list(video_scripts)
    if video_urls is None:
        raise ValueError("generate_digital_human_video_and_audio生成视频失败")
    for index, video_url in enumerate(video_urls):
        video_fragment = state.video_fragments[index]
        ...
        if video_fragment.action_type == "digital_human":
            ...
            video_data = get_url_data(video_url)
            with open(state.get_real_url(video_url_v2), "wb") as f:
                f.write(video_data)
            video_fragment.video_url_v2 = video_url_v2
            audio_path = os.path.join(video_fragment.id, "audio.mp3")
            extract_audio(
                state.get_real_url(video_url_v2), os.path.join(result_dir, audio_path), "mp3")
            ...
            digital_human_video_path = os.path.join(
                result_dir, video_fragment.id, "digital_human_video.mp4")
            video_data = get_url_data(video_url)
            with open(digital_human_video_path, "wb") as f:
                f.write(video_data)
            # 获取视频中的音频
            audio_path = extract_audio(
                digital_human_video_path, os.path.join(result_dir, video_fragment.id, "audio.mp3"), "mp3")
            # 判别音频是否超过了视频时长，假如超过了则文案要重新生成，并且记录视频时长，音频时长，音频文案
            audio_duration = get_audio_duration(state.get_real_url(audio_path))
            if audio_duration > video_fragment.video_duration:
                state.is_audio_too_long = True
                logger.error(
                    f"音频时长超过了视频时长，音频时长：{audio_duration}，视频时长：{video_fragment.video_duration}，音频文案：{video_fragment.video_script}")
                break
            video_fragment.audio_url = audio_path
            # 将音频添加到视频中
            video_url_v2 = os.path.join(video_fragment.id, "video_url_v2.mp4")
            merge_video_audio(state.get_real_url(video_fragment.video_url_v1), state.get_real_url(audio_path),
    ...
    for video_fragment in state.video_fragments:
        if video_fragment.action_type == "digital_human":
            ...
            video_v3_list.append(state.get_real_url(
                video_fragment.video_url_v3))
        else:
            video_v1_list.append(state.get_real_url(
                video_fragment.video_url_v1))
            video_v2_list.append(state.get_real_url(
                video_fragment.video_url_v2))
            video_v3_list.append(state.get_real_url(
                video_fragment.video_url_v3))
...
WORKFLOW_NODES = {
    ...
    "generate_digital_human_video_and_audio": generate_digital_human_video_and_audio,
...
def get_m2v_with_audio__digital_human_workflow():
    """
    获取应用
    """
    ...
    graph.add_conditional_edges(
        "generate_digital_human_video_and_audio", route_if_audio_too_long, {
...
async def ainvoke_m2v_with_audio_digital_human_workflow(user_id: str, id: str, product: str, product_info: str, model_images: list, video_fragment_duration: int, video_output_path: str):
    """
    调用m2v_workflow工作流
    Args:
        product: 商品名称
