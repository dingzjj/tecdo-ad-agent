...
def create_video_by_i2v(require: Annotated[str, Field(description="需求，具体需求，例如对视频的具体要求，例如：一个穿着白色连衣裙的女孩在海边跳舞")],
    ...
    if image_id.startswith("image"):
        ...
        # 使用素材库上的进行创作
        image: Optional[Material] = material_librarys[state["user_id"]
    ...
    if isinstance(image, Material):
        output_path = asyncio.run(
            model.generate(image_path=image.material_path, positive_prompt=require))
    else:
        output_path = asyncio.run(
...
def create_image_by_i2i(positive_prompt: Annotated[str, Field(description="正向提示词，具体需求，例如对图片的具体要求，例如：一个穿着白色连衣裙的女孩在海边跳舞")],
    ...
    if isinstance(image, Material):
        output_path = asyncio.run(
            model.generate(image_path=image.material_path, positive_prompt=positive_prompt))
    else:
        output_path = asyncio.run(
    ...
    user_id = state["user_id"]
    material_id = material_librarys[user_id].append_material_without_analysis(
        material_path=output_path, title=positive_prompt, description=positive_prompt)
    return f"生成图片成功,所用的提示词为{usage_feedback},生成的图片id为{f'{material_id}'}"
...
high_level_create_agent = create_react_agent(
    ...
    prompt=(
        "你是一名高级创作代理人员。可以完成广告，宣传短片等大型创作任务"
    ),
)
