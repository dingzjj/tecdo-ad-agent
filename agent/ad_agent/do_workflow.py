...
class M2VAgentState(BaseModel):
    ...
    type: str = Field(
        default="modify", description="类型,用于记录是生成还是修改")
    # generate_fragment
...
async def route_generate_or_modify(state: M2VAgentState, config):
    """
    路由生成或修改
    """
    # 根据用户的输入，判断是生成还是修改，使用llm分类器
    classifier = Classifier(
        categories=["生成,创建视频(不带音频字幕，数字人)", "生成，创建视频(带音频字幕，不带数字人)", "生成，创建视频(带音频字幕，带数字人)", "修改，编辑视频", "生成，创建片段", "素材预审"])
...
async def start_generate_video_with_audio(state: M2VAgentState, config):
    """
    开始生成视频(带音频字幕，不带数字人)
    """

    return {"type": "generate_video"}


async def generate_video_with_audio(state: M2VAgentState, config):
    pass


async def end_generate_video_with_audio(state: M2VAgentState, config):
    pass


# 生成，创建视频(带音频字幕，带数字人)
async def start_generate_video_with_audio_and_digital_human(state: M2VAgentState, config):
    """
    开始生成视频(带音频字幕，带数字人)
    """
    return {"type": "generate_video"}
...
async def generate_video_with_audio_and_digital_human(state: M2VAgentState, config):
    pass


async def end_generate_video_with_audio_and_digital_human(state: M2VAgentState, config):
    pass


# 素材预审
async def start_pre_review_material(state: M2VAgentState, config):
    """
    素材预审
    """
    return {"type": "pre_review_material"}


async def pre_review_material(state: M2VAgentState, config):
    """
    素材预审
    """
...
async def end_pre_review_material(state: M2VAgentState, config):
    """
    结束素材预审
    """
    ...
    state.chat_history.append(AIMessage(content=pre_review_material_content))
    return {"chat_history": state.chat_history}


async def start_generate_video(state: M2VAgentState, config):
    """
    开始生成
    """
    # 附加信息中的图片信息必定作为模特图片
    ...
    return {"workflow_state": workflow_state, "type": "generate_video"}


async def gv_ainvoke_m2v_workflow(state: M2VAgentState, config):
    """
    调用m2v_workflow
    """
...
async def end_generate_video(state: M2VAgentState, config):
    """
    结束生成
    """
    video_content = ""
...
async def start_generate_fragment(state: M2VAgentState, config):
    """
    开始生成片段
    """
    return {"type": "generate_fragment"}


async def generate_fragment(state: M2VAgentState, config):
    """
    生成片段
...
async def end_generate_fragment(state: M2VAgentState, config):
    """
    结束生成片段
    """
    fragment_content = ""
...
async def gf_ainvoke_m2v_workflow(state: M2VAgentState, config):
    """
    调用m2v_workflow
    """
    workflow_state = state.workflow_state
...
async def start_modify(state: M2VAgentState, config):
    """
    开始修改,判断是否缺少某些条件
    """
    state.type = "modify"
    if state.workflow_id.strip() == "":
        state.chat_history.append(AIMessage(content="请先输入视频id"))
        return {"chat_history": state.chat_history, "type": "modify"}
    return {"type": "modify"}


async def m_load_state(state: M2VAgentState, config):
...
async def m_generate_operations(state: M2VAgentState, config):
    """
    生成更新操作
    """
    # 初始化
    ...
    if state.error_info is None:
        ...
        update_operations = await m2v_agent.regenerate_update_operations(
            state.chat_history, state.error_info.message, state.update_operations)
        return {"update_operations": update_operations}


async def m_execute_operations(state: M2VAgentState, config):
...
async def supply_information(state: M2VAgentState, config):
    """
    人机交互：补充信息
    """
    # 初始化
    ...
    state.chat_history.append(AIMessage(content=hint))
    state.chat_history.append(HumanMessage(content=user_input))
    # 返回用户输入
    return {"chat_history": state.chat_history}


def route_error_info(state: M2VAgentState, config):
    """
    路由错误信息
    """
...
async def m_evaluate_video_fragments(state: M2VAgentState, config):
    """
    评估视频片段
    """
...
async def m_video_stitching(state: M2VAgentState, config):
    """
    视频拼接,添加转场效果
...
async def m_save_state(state: M2VAgentState, config):
    """
...
async def end_modify(state: M2VAgentState, config):
    """
    结束修改
    """
    state.chat_history.append(
        AIMessage(content="视频修改完成,修改后的视频id为" + state.workflow_state.id))
    return {"chat_history": state.chat_history}
...
def get_do_workflow_app():
    ...
    获取app
    """
    graph = StateGraph(M2VAgentState)
    # 生成视频
