...
from agent.third_part.amt.networks.blocks.raft import (
    coords_grid,
    BasicUpdateBlock, BidirCorrBlock
)
from agent.third_part.amt.networks.blocks.feat_enc import (
    LargeEncoder
)
from agent.third_part.amt.networks.blocks.ifrnet import (
    resize,
    Encoder,
    InitDecoder,
    IntermediateDecoder
)
from agent.third_part.amt.networks.blocks.multi_flow import (
    multi_flow_combine,
    MultiFlowDecoder
)
...
class Model(nn.Module):
    ...
    def _corr_scale_lookup(self, corr_fn, coord, flow0, flow1, embt, downsample=1):
        ...
        corr0, corr1 = corr_fn(coord + flow1 * t1_scale,
    ...
    def forward(self, img0, img1, embt, scale_factor=1.0, eval=False, **kwargs):
        mean_ = torch.cat([img0, img1], 2).mean(1, keepdim=True).mean(
        ...
        fmap0, fmap1 = self.feat_encoder(
            [img0_, img1_])  # [1, 128, H//8, W//8]
        ...
        delta_ft_2_, delta_up_flow_3 = self.update3_high(
            ft_2_, up_flow_3, corr_3)
        ft_2_ += delta_ft_2_
        ...
        delta_ft_1_, delta_up_flow_2 = self.update2_high(
            ft_1_, up_flow_2, corr_2)
        ft_1_ += delta_ft_1_
        ...
        if scale_factor != 1.0:
            up_flow0_1 = resize(up_flow0_1, scale_factor=(
                1.0/scale_factor)) * (1.0/scale_factor)
            up_flow1_1 = resize(up_flow1_1, scale_factor=(
                1.0/scale_factor)) * (1.0/scale_factor)
