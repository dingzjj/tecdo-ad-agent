...
from agent.exception import Kling2_1_I2VError
...
async def run_kling2_1_i2v(img_path: str, positive_prompt: str, negative_prompt: str, duration: Literal[5, 10], output_dir: str):
    ...
    if (not img_path.lower().endswith((".png", ".jpg", ".jpeg"))
        or image_size > 10 * 1024 * 1024
        or image_width < 300
        or image_height < 300
        or image_width / image_height < 0.4
        or image_width / image_height > 2.5
        ):
        raise Kling2_1_I2VError(
            "图片格式不正确或图片大小不符合要求，请上传PNG或JPG格式的图片，大小不超过10MB，宽高比在0.4到2.5之间，宽高均大于300px。")
    ...
    logger.info(f"kling2_1_i2v payload: {payload}")
    headers = {
    ...
    while True:
        try:
            ...
            raise Kling2_1_I2VError(f"请求异常: {type(e).__name__}: {e}")
        except httpx.HTTPStatusError as e:
            logger.error(f"请求失败，状态码：{e.response.status_code}")
            raise Kling2_1_I2VError(f"请求失败，状态码：{e.response.status_code}")
        except Exception as e:
            logger.error(f"解析响应失败: {e}")
            raise Kling2_1_I2VError(f"解析响应失败: {e}")
        task_status = data.get("task_status")
        if time.time() - start_time > max_wait:
            logger.error("等待超时，任务未完成。")
            raise Kling2_1_I2VError("等待超时，任务未完成。")
        if task_status == "processing":
            ...
            if video_list:
                ...
                if url:
                    ...
                    logger.error("视频结果为空。")
                    raise Kling2_1_I2VError("视频结果为空。")
            else:
                logger.error("视频结果为空。")
                raise Kling2_1_I2VError("视频结果为空。")
